...
import {
  getUsersProfessionals,
  getUsersProfessionalsInactives,
  postUsersProfessionals,
  updateUsersProfessionals
...
export function* onGetAll() {
  try {
    const { data: actives } = yield call(getUsersProfessionals);
    const { data: inactives } = yield call(getUsersProfessionalsInactives);
    yield put(getAllSuccess(actives));
    yield put(getAllInactivesSuccess(inactives));
  } catch (error) {
