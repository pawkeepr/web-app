    ...
    return (
        <section className="flex flex-col justify-start p-4 w-full">
            <div className="gap-2 flex-wrap flex flex-col mt-2 w-full justify-between ">
                {pet && <p className="text-gray-500 flex justify-between">
                    <strong className="mr-2">Pet:</strong>
                    <span>
                        {
                            `${pet.name_pet}, ${Species[(pet.specie as Species) as unknown as KeyOfSpecies]}, ${pet.race as string}`
                        }
                    </span>
                </p>}
                {name_tutor && <p className="text-gray-500 flex justify-between">
                    <strong className="mr-2">Tutor:</strong>
                    {name_tutor}
                </p>}
                {cpf_tutor && <p className="text-gray-500 flex justify-between">
                    <strong className="mr-2">CPF:</strong>
                    {cpf_tutor}
                </p>}
                {email_tutor && <p className="text-gray-500 flex justify-between">
                    <strong className="mr-2">Email:</strong>
                    {email_tutor}
                </p>}
                {phone_tutor && <p className="text-gray-500 flex justify-between">
                    <strong className="mr-2">Telefone:</strong>
                    {phone_tutor}
                </p>}
                {whatsapp_tutor && <p className="text-gray-500 flex justify-between">
